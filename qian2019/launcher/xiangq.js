...
var JS_MSG_5 = JS_MSG_5 || '' ;
var JS_MSG_6 = JS_MSG_6 || '' ;
var JS_MSG_7 = JS_MSG_7 || '' ;
var JS_MSG_8 = JS_MSG_8 || '' ;
var JS_MSG_9 = JS_MSG_9 || '' ;
var JS_MSG_10 = JS_MSG_10 || '' ;
var JS_MSG_11 = JS_MSG_11 || '' ;
var JS_MSG_12 = JS_MSG_12 || '' ;
var JS_MSG_13 = JS_MSG_13 || '' ;
var JS_MSG_14 = JS_MSG_14 || '' ;
var JS_MSG_21 = JS_MSG_21 || '' ;
var JS_MSG_74 = JS_MSG_74 || '' ;
var JS_MSG_75 = JS_MSG_75 || '' ;
var JS_MSG_76 = JS_MSG_76 || '' ;
var JS_MSG_77 = JS_MSG_77 || '' ;
var JS_MSG_78 = JS_MSG_78 || '' ;
var JS_MSG_79 = JS_MSG_79 || '' ;
var JS_MSG_80 = JS_MSG_80 || '' ;
var JS_MSG_81 = JS_MSG_81 || '' ;
var JS_MSG_82 = JS_MSG_82 || '' ;
var JS_MSG_83 = JS_MSG_83 || '' ;
var JS_MSG_84 = JS_MSG_84 || '' ;
var JS_MSG_85 = JS_MSG_85 || '' ;
var JS_MSG_86 = JS_MSG_86 || '' ;
var JS_MSG_87 = JS_MSG_87 || '' ;
var JS_MSG_88 = JS_MSG_88 || '' ;
var JS_MSG_89 = JS_MSG_89 || '' ;
var JS_MSG_90 = JS_MSG_90 || '' ;
var JS_MSG_91 = JS_MSG_91 || '' ;
var JS_MSG_92 = JS_MSG_92 || '' ;
var JS_MSG_93 = JS_MSG_93 || '' ;
var JS_MSG_94 = JS_MSG_94 || '' ;
var JS_MSG_95 = JS_MSG_95 || '' ;
var JS_MSG_96 = JS_MSG_96 || '' ;
var JS_MSG_97 = JS_MSG_97 || '' ;
var JS_MSG_98 = JS_MSG_98 || '' ;
var JS_MSG_99 = JS_MSG_99 || '' ;
var JS_MSG_100 = JS_MSG_100 || '' ;
var JS_MSG_101 = JS_MSG_101 || '' ;
var JS_MSG_102 = JS_MSG_102 || '' ;
var JS_MSG_103 = JS_MSG_103 || '' ;
var JS_MSG_104 = JS_MSG_104 || '' ;
var JS_MSG_105 = JS_MSG_105 || '' ;
var JS_MSG_106 = JS_MSG_106 || '' ;
var JS_MSG_107 = JS_MSG_107 || '' ;
var JS_MSG_108 = JS_MSG_108 || '' ;
var JS_MSG_109 = JS_MSG_109 || '' ;
var JS_MSG_110 = JS_MSG_110 || '' ;
var JS_MSG_111 = JS_MSG_111 || '' ;
var JS_MSG_112 = JS_MSG_112 || '' ;
...
function setFlowTipsByTime(msg,time) {
		if( !this.fTips ){
			this.fTips = $("<div class='login_flow_box'></div>").appendTo($(document.body));		
		}
		this.fTips.html(msg).show();
...
function re_nickname(nickname){  //记录当前玩家的昵称
	if(arguments.length == 0){
		 re_nickname_score.nickname = JS_MSG_74;
	}else{
...
function setShareStatement(){
	...
	if(nickname == JS_MSG_74){
		SHARE_TITLE = nickname+JS_MSG_75+"【"+GAMENAME+"】"+JS_MSG_76;
	}else{
		SHARE_TITLE = nickname+JS_MSG_75+"【"+GAMENAME+"】"+JS_MSG_77+score;
	}
}
	...
	$.ajax({
			...
			success: function(ret){ 
				if(ret=='over_duang'){
					...
					setFlowTips(JS_MSG_79);
				}else if(ret =='duang_over'){
					setFlowTips(JS_MSG_80);
				}else{
					
					setFlowTips(JS_MSG_81+ret+ JS_MSG_82);
					var DUAND_ID='#duang'+duangid+' img';
					$(DUAND_ID).attr('src','/resources/images/meiribisai/duang1.png');
					$(that).find('img').attr('src','/resources/images/meiribisai/duang1.png');
				}
...
$(function(){
				...
				if(ret == 'ok'){
				  ...
				  if(typeof self.loginCallback == 'function'){
					  try{
				        ...
				        $("#txtPwd").val('');
						self.setFlowTips(JS_MSG_8);
						return;
						
					}
				else if(ret=='restricted'){
					setTimeout(function(){self.setFlowTips(JS_MSG_114);},500);
					 return;
				}
	...
	if( /ipad|ipod|iphone|android/i.test(navigator.userAgent) ){
		...
		$("#w_laren,#w_laren img").bind('touchstart', function(event){
			event.stopPropagation();
		}).bind('touchend',function(event){
			//传奇世界，流量包页面
			if (contains(STREAM_FLOW,GAMEID)) {
				streamFlow();
			} else {
				_czc.push(['_trackEvent','邀请功能（游戏内）','展示人数','','','']);
				getReceiveBox(0);
			}
			event.stopPropagation();
		});
		...
		$("#w_laren,#w_laren img").bind('click', function(event){
			//传奇世界，流量包页面
			if (contains(STREAM_FLOW,GAMEID)) {
				streamFlow();
			}
    ...
    $("#s14").click(function(e){
	 ...
	 $("#s13").click(function (e) {
		...
		if (ISLOADING) return;
    	showLoading();
		if(METHODS.html()==JS_MSG_85 || METHODS.html()!=''){
			hideLoading();
		}else{
    	 $.ajax({
...
function paihangbang_data(data,n){
   ...
   // var current = data[i].sex == 0 ? 'huise'  : (data[i].sex == 1 ? 'blue' : 'red');
    ...
    str_pai+="<p class='mingc'>"+JS_MSG_21+"</p>";	
    }                                                                                                                 
	...
	if(i>99){
		str_pai+="<div class='before_100_show' style='padding-top:5%; width:90%; margin:auto; color:#d86c12;text-align:center;'>"+JS_MSG_86+"</div></div>";
    ...
    str_pai+="</div>";
    str_pai+="</div>";
    str_pai+="<div class='haoyou'>"+JS_MSG_87+"</div>";
		setTimeout(function(){
		$('.before_100_show').css('display','none');
		},2000);
	}
...
var userid=USERID || 0; 
        ...
        for(var y=0;y<data.length;y++){
        	...
        	if(data[y].userid==userid && window.is_myinfo==1){
                ...
                str_pai_2+="<p class='mingc'>"+JS_MSG_21+"</p>";	
                }                                                                 
      ...
      $("#input_text").focus(function(){
		  if($('#input_id').attr('value') =='0'){
				...
				if($(this).val()!="")
                {
					...
					$(this).attr('placeholder',JS_MSG_88);
				}
		  }
       }).blur(function(){
			 if($('#input_id').attr('value') =='0'){
   ...
   function getComent(){
    ...
    function showComment(data) {
    	...
    	for (var i in data) {
    		...
    		var name = obj.nickname == "" ? JS_MSG_90 : obj.nickname;
            //评论名称截取字符串
			 ...
			 if($("#input_id").attr('value') != '0'){
			  ...
			  $("#input_text").focus(function(){
					  ...
					  if( $("#input_text").val()!=''){
						...
						$("#input_text").attr('placeholder',JS_MSG_88);
		...
		// 提交评论   
          var commentTime = 0;
			...
			$(".xpinglu1_a_3 .bottom .xiaolian").click(function(e){
				...
				if( comment == "" ){
					msgBoxShow(JS_MSG_94);
					return false;
				}
				if( comment == JS_MSG_88 ){
					msgBoxShow(JS_MSG_95);
					return false;
				}
				if(getBytesCount(comment)>80){
					msgBoxShow(JS_MSG_96);
					return false;
				}
				var now = (new Date()).getTime();
				if( now - commentTime < 10000){
					msgBoxShow(JS_MSG_97);
					return false;
				}
				if (ISLOADING) return;
				showLoading();
				...
				$.ajax({
					...
					success : function(data){
						...
						getComent();
						if(reply_id>0){
						 msgBoxShow(JS_MSG_99);
						}else{
						 msgBoxShow(JS_MSG_100);
						}
						$("#input_id").attr('value','0');
...
function close_shung_shier(){
	if(USERID > 0){
		$.ajax({
			url:"/?r=Play/SetActCache/gameid/"+GAMEID,
			dataType: "",
			success: function(){
...
(function(){
 if(isMobile()){
	...
	if(window.is_activity){
			...
			if(REBATE_TOP && REBATE_WIDTH && REBATE_MINHEIGHT && REBATE_ACTIMG){
				var matchNode = document.createElement("div");
				matchNode.className='shung_shier';
				matchNode.style.cssText="position:absolute; top:0; width:100%; height:100px; z-index:999999999; overflow:auto;";
				
				matchNode.innerHTML='<div class="shungshier" style="width:100%; max-width:640px; margin:auto; position:relative;"><div class="shungshier1"><img style="width:100%; height:auto;" src="'+REBATE_ACTIMG+'" width="640" height="960"><div class="anniu " onclick="close_shung_shier();" style=" width:'+REBATE_WIDTH+'%; min-height:'+REBATE_MINHEIGHT+'rem; top:'+REBATE_TOP+'%;position:absolute;left:50%; background:rgba(0,0,0,0); transform:translateX(-50%); -webkit-transform:translateX(-50%); -moz-transform:translateX(-50%);"></div></div>';
				
				
				document.body.appendChild(matchNode);
				var clientH= window.innerHeight|| document.documentElement.clientHeight|| document.body.clientHeight;
				$(".shung_shier").height(clientH);
				...
				if(CLOSEREBATE){
					...
					matchNode.innerHTML = '<div class="bisaixxa"><div class="top" ><img src="/resources/images/xiangq/title.png" width="640" height="78"></div><div class="contentt"><img src="/resources/images/xiangq/bg_beijing.png" ><div class="content1"><div class="content1_a">'+JS_MSG_101+'</div><div class="content1_b"><img src="'+window.activity_img+'" width="164" height="168"></div><div class="content1_c">'+GAMENAME+'</div><div class="content1_d"><div class="content1_d_1">'+JS_MSG_102+'</div><div class="content1_d_2"><ul><li>'+JS_MSG_103+'</li><li>'+JS_MSG_104+'</li><li>'+JS_MSG_105+'</li></ul></div></div><div class="content1_d"><div class="content1_d_1">'+JS_MSG_106+'</div><div class="qunhao">'+JS_MSG_107+'</div></div><div class="content1_d"><div class="content1_d_1">'+JS_MSG_108+'</div><div class="content1_d_2"><ul><li>'+JS_MSG_109+'</li><li>'+JS_MSG_110+'</li><li>'+JS_MSG_111+'</li></ul></div></div><div class="kaishianniu" id="hdstrat"><img src="/resources/images/xiangq/btn_start.png" width="320" height="70" id="hd_strat"></div></div></div></div>';
					
					document.body.appendChild(matchNode);
					$(".kaishianniu").live("click", function(){$(".bisaixx").css('display','none')});
					$("#hdstrat").on("click", function(){$(".bisaixx").css('display','none')});
				}
			}
	        if(SHOWACTIVITY){
				$(".shung_shier").show();
...
coverNode.innerHTML = '<div class="load1" style="z-index:1000000;"><img  src="/resources/images/xijie/loading.png" width="411" height="349" style="z-index:999999999;"></div><div class="load2" style="z-index:1000000;">&copy; 2015 68微游戏</div>'
...
function succShareCallback(){ 
    if(window.cps_name == 'yinyue'){
		if(isWeixin()){
			 var randNum = Math.floor(Math.random()*100);
			 if(randNum > 50){
				window.location.href='http://dwz.cn/2NGpD1';
			 }else{
			 	window.location.href='http://s1.eqxiu.com/eqs/link?id=32372414&url=http%3A%2F%2Fdwz.cn%2F2NIPUy';
			 }
		}
		return; 
	}
			...
			if(isIos()){
				...
				if(isWeixin()){
				  ...
				  // setTimeout(function(){
					...
					//  	window.location.href = window.appdownload;
					// }else{
...
//跳转到下载app
function _appdownloadaddress(){
	_czc.push(["_trackEvent","APP下载弹框","下载app","","",""]);
	addCookie("_appdown"+GAMEID,1,1);
	if(!isMobile()){
		window.location.href=_APPDOWNLOADADDRESS;
	}else{
		if(isWeixin()){
            window.location.href="http://a.app.qq.com/o/simple.jsp?pkgname=com.play68&f=wyct";
		}else{
			window.location.href=_APPDOWNLOADADDRESS;
		}
	}
...
//移除app下载窗口
function remove_appdownload(){
	_czc.push(["_trackEvent","关闭APP下载弹框","关闭APP下载弹框","","",""]);
	//addCookie("_appdown"+GAMEID,1,1);
	$("#appdownload").remove();
}
//app游戏页获取礼包
function appGetlibao(){
...
function getGameId(){
	if( window.Play68app){ // 68app
		...
		if(window.Play68app.getGameId) {
			
			window.Play68app.getGameId(String(GAMEID));
		}
 ...
 * 宝箱奖励 领取框
 */
function getReceiveBox(flag) {
	var flag = flag || 0;
	if (flag) {
		$('.w_laren .libao2').remove();
		$.post('/index.php?r=Play/getReceiveBox',{gameid:GAMEID,flag:flag},function(data,status){
			$(".w_laren .libao1").after(data);
			isHasBox();
		});
	} else {
		/*$('.w_laren').remove();
 ...
 */
function isHasBox() {
	$.post('/index.php?r=Play/hasBox',{gameid:GAMEID},function(data,status){
		if (data.errcode == 1) {
			$('#w_laren span').remove();
			var str  = '<span style="left: 70%;"></span>';
			$('#w_laren').append(str);
		} else {
			$('#w_laren span').remove();
		}
 ...
 */
function streamFlow() {
	$.post('/index.php?r=Play/streamFlow',{gameid:GAMEID},function(data,status){
		$('.liuliangbao').remove();
		$('body').append(data);
	});
