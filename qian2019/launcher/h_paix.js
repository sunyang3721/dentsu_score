...
$(function(){
		  ...
		  $(".h_paix_3,.h_paix_4").click(function(){
						back();
    		...
    		$.ajax({
    			...
    			success: function(data) {
					...
					if ($('#footer_content').length > 0) {
						...
						var htmlspan4 = '<span  class="span4 center_box" onClick="show_share_page();stopBubble(event);"><img src="/resources/images/h_paix/xuanyao.png" width="81" height="34"></span>';
						$('.footer .span3').after(htmlspan4);
					...
					$(".footer .myliaotian").css('display','none'); 
    			},
        		complete: function() {
           			hideLoading();
           		}
	   ...
	   window.horizontalMenuForMessageList = function() {
		    if (ISLOADING) return;
		 	showLoading();
		 	_czc.push(["_trackEvent","聊天功能","消息按钮","","",""]);
	   		$.ajax({
		   ...
		   showLoading();
	   		$.ajax({
	   			...
	   			success: function(data) {
					...
					$(".footer .content1").hide();
					$('#horizontalMenu').css('visibility','visible');
					$(".footer .myliaotian").css('visibility','hidden');
	   			 },
	    		complete: function() {
	       				hideLoading();
	       		}
	...
	//好友列表
	window.friendList = function() {
		showLoading();
		$.ajax({
			url: '/?r=personal/friendList',
			type:'POST',
			data: {flag:1},
			success: function(data) {
				$('.empty').html(data);
