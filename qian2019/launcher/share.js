...
if (YQM > 0 && USERID > 0) {
    ...
    if (/68_yqm_play/i.test(GAMEURL)) {
        GAMEURL = changeURLArg(GAMEURL, '68_yqm_play', YQM);
    } else {
        if (GAMEURL == LOCATION_HOST) {
            GAMEURL = GAMEURL + '?r=index/index&68_yqm_play=' + YQM;
        } else {
            if (GAMEURL.indexOf('?') > 0) {
                GAMEURL = GAMEURL + '&68_yqm_play=' + YQM;
            } else {
                GAMEURL = GAMEURL + '?68_yqm_play=' + YQM;
            }
        }
...
//游戏页内
function is_in_game() {
    if (/games|game|\/play\/|\/g/i.test(location.href)) {
        return true;
    }
}
        ...
        $.ajax({
            ...
            success: function (data) {
                if (is_in_game()) {//游戏页内
                    ...
                    $('#ewm_share_headimg img').attr('src', data.headimg.replace("http://","//"));
                    ...
                    if (isIos()) {
                        ...
                        if (isWeixin() == true) {
                            ...
                            $('#ewm_share_url img').attr('src', '/?r=site/2wm&content=' + window.appdownload);
                        }
                    }
...
function JiaThis_Button() {
    ...
    var f = function (node, imgSrc) {
        ...
        node.css({
            'background': 'url(' + imgSrc + ') no-repeat left',
            'text-align': 'left',
            'overflow': 'hidden',
...
function checkMobile() {
    if (isMobile()) {
        ...
        if(myrules && myrules.length) for (i = 0; i < myrules.length; i++) {
...
function ranking(data) {
    $(window).trigger('playGameEnd', [data]);
    if (ISACTGAME) {
        if (window.abletoplay) {
            if (window.freeplay) {
            } else {
                actrank(data);
            }
...
//					if (IS_FROM_SHARE == 1 &&  isNetGame != 1 && HOUSEHOLDER) {
                            challengeSuccess();
                            return;
                        }
...
//					if (IS_FROM_SHARE == 1 &&  isNetGame != 1 && HOUSEHOLDER) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (window.cps_name == 'xixi') {
                    return;//‘xixi’cps不弹排行榜
                }
                //	if(LocalStorageKeys){
...
//指定条件弹出游戏广告
function bannerinplay_show() {
    console.log('sd');
    if (IS_NETGAME == '1' || IS_ACTIVITY) {

    } else {
        if (!!window.videoGGShowReplay && !!window.bannerTime) {
            if (Date.now() - window.bannerTime > 180000) {
                window.videoGGShowReplay();
                window.bannerTime = Date.now();
            }
        }
        // if(window.bannerTime){
        // 	$("#bannerinplay").css('display','block');
        // 	$(".xuanzhuanyouxi1").css('display','block');
        // 	window.bannernoshow = false;
        // 	if(window.bannerinplay){
        // 		if(window.bannernoshow){
        //
        // 		}else{
        // 			$("#bannerinplay").css('display','block');
        // 			$(".xuanzhuanyouxi1").css('display','block');
        // 			if(ALLBANNERNUM > 1){
        // 				var BANNER_NUM = parseInt(ALLBANNERNUM*Math.random());
        // 				$("#bannerinplay_img").attr('src',ALLBANNER[BANNER_NUM]['bannerurl']);
        // 				window.bannerinplayname = ALLBANNER[BANNER_NUM]['descs'] ;
        // 				window.bannerinplayurl  = ALLBANNER[BANNER_NUM]['url_info'] ;
        // 			}
        // 		}
        // 	}
        // 	window.bannerinplay = false;
            ...
            if (isIos()) {
                ...
                if (isWeixin()) {
                    ...
                    if (IOSplay68() || window.iosplay68) {
                        ...
                        window.location.href = window.iosdownload;
                    }
                }
            } else {
                if (isWeixin()) {
...
function onmessage(e) {
    var _fns = {
        ...
        'setShareInfo': function (args) {
            setShareInfo(args);
        },
        'setHorizontal': function (args) {
            HORIZONTAL = args;
        },
        'ranking': function (args) {
            ranking(args);
        },
        'showRanking': function (args) {
            showRanking();
        },
        'closeRecommend': function (args) {
            close_Recommend();
        },
        'festivalgame': function (args) {
            festival_game();
        },
        'ly_set_share': function (args) {
            ly_set_share(args);
        },
        'set_share_url': function (args) {
            set_share_url(args);
        },
        'setCrazycaitu': function (args) {
            setCrazycaitu(args);
        },
        'getCrazycaitu': function (args) {
            getCrazycaitu(args);
        },
        'getStorage': function (args) {
            getStorage(args);
        },
        'set_share_icon': function (args) {
            set_share_icon(args);
        },
        'resetView': function () {
            resetView();
