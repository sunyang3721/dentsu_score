$(function() {
	...
	//作品总数量
	$.ajax('queryWorksInfoList.do',{
		dataType:'json',
		type:'post',
		success:function(data){
			var contshow = 'Search from '+data['worksCnt']+' works';
			$('#keywork').attr('placeholder',contshow);
		}
	})
					...
					var adhtml = "";
						...
						$('.adqueding').on('click',function(){
							...
							var adhtmls = '<li class="'+splits(adFestId,0)+' ad_id_list"><ul class="list-inline adcheck">' 
								...
								adhtmls += '<li data-id="'+splits(adFestId,0)+'" class="adremove"><span class="btn btn-xs btn-danger"><i class="glyphicon glyphicon-remove"></i></span></li>';
								adhtmls += '</li></ul>';
			...
			if(worklink){
				$('.worklinks').each(function(){
					...
					if(linkUrl=='' || linkName=='' ){
						...
						var bo = {linkId:'',linkName:linkName,linkUrl:linkUrl};
						worksLink.push(bo);
					}
				});
			}
